...
vec4 vec4::operator*(const mat4x4 &m) {
  ...
  r.x = m.m[0][0] * x + m.m[1][0] * y + m.m[2][0] * z + m.m[3][0] * w;
  r.y = m.m[0][1] * x + m.m[1][1] * y + m.m[2][1] * z + m.m[3][1] * w;
  r.z = m.m[0][2] * x + m.m[1][2] * y + m.m[2][2] * z + m.m[3][2] * w;
  r.w = m.m[0][3] * x + m.m[1][3] * y + m.m[2][3] * z + m.m[3][3] * w;
  return r;
}
...
mat4x4 Transpose(const mat4x4 &m) {
  return mat4x4{
      m.m[0][0], m.m[1][0], m.m[2][0], m.m[3][0], m.m[0][1], m.m[1][2],
      m.m[2][2], m.m[3][1], m.m[0][2], m.m[1][2], m.m[2][2], m.m[3][2],
      m.m[0][3], m.m[1][3], m.m[2][3], m.m[3][3],
  };
}
float Determinant(const mat4x4 &m) {
  return m.m[0][0] * m.minor(0, 0).det() - m.m[0][1] * m.minor(0, 1).det() +
         m.m[0][2] * m.minor(0, 2).det() - m.m[0][3] * m.minor(0, 3).det();
}
